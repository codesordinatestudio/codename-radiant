...
- **Golden rule (Language-Agnostic Native Compilation):** Radiant is fundamentally language-agnostic. The `.radiant` DSL compiles into a Universal AST, which is then consumed by the target language's native runtime. For TypeScript, this lowers directly to Bun's native runtime; for Go, it lowers to native Go; for Python, native Python. Compile-time abstractions must always lower directly to the language's native runtime behavior wherever possible. Avoid carrying Radiant-only syntax or framework indirection into the compiled server hot paths unless there is absolutely no practical native equivalent for that target language.
