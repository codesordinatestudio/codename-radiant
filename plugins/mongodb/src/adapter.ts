...
import { logger, generateId, INTERNAL_SYSTEM_COLLECTIONS } from "@codesordinatestudio/lucent-core";
import { buildTable, type TableDefinition, type ColumnDefinition } from "@codesordinatestudio/lucent-core";
...
const DDL_MARKER = "--__lucent_mongo__:";
...
export class MongoDBAdapter implements LucentAdapter {
  ...
  async connect(): Promise<void> {
    ...
    logger.info({ database: this._databaseName }, "Connected to MongoDB");
  }
  ...
  getSystemTableStatements(): string[] {
    return INTERNAL_SYSTEM_COLLECTIONS.map((collection) => this.createTableDDL(buildTable(collection)));
  }
  ...
  async getCurrentSchema(): Promise<{ tables: string[]; columns: Record<string, string[]> }> {
    ...
    for (const info of collections) {
      ...
      if (!name || name.startsWith("lucent_")) continue;
  ...
  async create(collection: string, data: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    doc._id = (providedId as string | undefined) ?? generateId();
  ...
  private runtimeFieldToStore(collection: string, field: string): string {
    ...
    if (collection.startsWith("lucent_")) return assertMongoFieldPath(field);
  ...
  private storeFieldToSchema(collection: string, field: string): string {
    ...
    if (collection.startsWith("lucent_")) return field;
