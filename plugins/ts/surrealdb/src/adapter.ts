...
function generateCreateTableSurreal(table: TableDefinition): string {
  return `DEFINE TABLE ${table.name} SCHEMALESS;`;
}

function generateAddColumnSurreal(table: string, column: ColumnDefinition): string {
  return `DEFINE FIELD ${column.name} ON ${table} TYPE any;`;
}

function generateRenameColumnSurreal(table: string, oldName: string, newName: string): string {
...
export class SurrealDBAdapter implements RadiantAdapter {
  ...
  private async _connectInner(): Promise<void> {
    ...
    await this.db.use({
      namespace: this.options.namespace ?? "radiant-bun",
      database: this.options.database ?? "radiant-bun",
    });
  ...
  async find(collection: string, query: QueryArgs): Promise<PaginatedResult> {
    ...
    if (cursor) {
      ...
      console.debug({ dataSql, params }, "Executing SurrealDB cursor find query");
    ...
    console.debug({ dataSql, countSql, params }, "Executing SurrealDB find query");
  ...
  async count(collection: string, query?: Pick<QueryArgs, "where">): Promise<number> {
    ...
    const countSql = `SELECT count() AS total FROM ${surrealIdentifier(collection)}${whereSql} GROUP ALL`;
    console.debug({ countSql, params }, "Executing SurrealDB count query");
  ...
  async findById(collection: string, id: string): Promise<Record<string, unknown> | null> {
    console.debug({ collection, id }, "Executing SurrealDB findById query");
  ...
  async findByIds(collection: string, ids: string[]): Promise<Record<string, unknown>[]> {
    ...
    console.debug({ collection, count: ids.length }, "Executing SurrealDB findByIds query");
  ...
  async create(collection: string, data: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    console.debug({ collection, id, fields: Object.keys(doc) }, "Executing SurrealDB create");
  ...
  async update(collection: string, id: string, data: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    console.debug({ collection, id, fields: Object.keys(updateData) }, "Executing SurrealDB update");
  ...
  async delete(collection: string, id: string): Promise<void> {
    console.debug({ collection, id }, "Executing SurrealDB delete");
  ...
  async deleteMany(collection: string, ids: string[]): Promise<void> {
    if (ids.length === 0) return;
    console.debug({ collection, count: ids.length }, "Executing SurrealDB bulk delete");
